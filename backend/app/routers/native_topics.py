...
ITEM_PATH = "/{native_language}/{topic_id}"


@router.get("/{native_language}", response_model=List[NativeTopicHeader])
async def get_all(
    service: NativeTopicServiceDep,
    language: Language,
    level: Level,
    ...
    return JsonStreamingResponse(service.get_list(language, level, native_language, username))
...
async def translate_topic(
    ...
    native_topic = await translator.translate_topic_to_native(native_language, topic_id)
    return await native_topic_service.create(native_topic)


@router.get(ITEM_PATH, dependencies=[Depends(get_topic_for_user)])
async def get(service: NativeTopicServiceDep, native_language: Language, topic_id: UUID) -> NativeTopic:
